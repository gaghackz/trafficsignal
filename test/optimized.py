...
TOTAL_CARS = 400  # SYNCED ASYMMETRIC GRIDLOCK CAP
...
class StatsLogger:
    def __init__(self):
        ...
        print("🚥 Stats Logger Initialized. True Micro-Step PSO Benchmark Running...")
    ...
    def print_final_report(self):
        ...
        print("\n" + "="*50)
        print("🚦 TRUE PSO OPTIMIZED SIMULATION - FINAL STATISTICS 🚦")
        print("="*50)
...
# ==========================================
# 4. PARTICLE SWARM OPTIMIZER (Micro-Step Engine)
# ==========================================
class ParticleSwarmOptimizer:
    def __init__(self, num_particles=20, iterations=15):
        self.num_particles = num_particles
        self.iterations = iterations
        self.w, self.c1, self.c2 = 0.5, 1.5, 1.5
        self.bounds = [(10, 25), (25, 45), (20, 50), (60, 100), (15, 30), (35, 60)]
        self.particles, self.velocities = [], []
        self.pbest_pos, self.pbest_fit = [], []
        self.gbest_pos = None
        self.gbest_fit = -1.0
    ...
    def fitness(self, genome):
        # TRUE MICRO-STEP SIMULATOR: 1 Loop = 1 Second of physics.
        try:
            flc = build_fuzzy_system(genome)
            total_delay = 0.0
            queues = [0.0, 0.0, 0.0, 0.0]
            max_wait = [0.0, 0.0, 0.0, 0.0]
            spawned = 0
            ...
            # Simulate exactly 400 cars arriving 5 per second asymmetrically
            for sim_sec in range(1200): # Hard cap at 20 mins sim time
                # Spawn Physics (5 cars/sec total)
                if spawned < TOTAL_CARS:
                    queues[NORTH] += 2.25
                    queues[SOUTH] += 2.25
                    queues[EAST] += 0.25
                    queues[WEST] += 0.25
                ...
                # FLC Decision & Phase Machine
                if state == "GREEN":
                    if timer == 0:
                        d1, d2 = (0, 1) if current_phase == 0 else (2, 3)
                        o1, o2 = (2, 3) if current_phase == 0 else (0, 1)
                        try:
                            flc.input['active_traffic'] = min(queues[d1] + queues[d2], 50)
                            flc.input['competing_wait'] = min(max(max_wait[o1], max_wait[o2]), 120)
                            flc.compute()
                            green_duration = max(MIN_GREEN, min(flc.output['green_time'], MAX_GREEN))
                        except:
                            green_duration = MIN_GREEN

                    # Departure Physics (Pygame clears ~2.5 cars per lane per sec)
                    d1, d2 = (0, 1) if current_phase == 0 else (2, 3)
                    if queues[d1] > 0: queues[d1] = max(0, queues[d1] - 2.5)
                    if queues[d2] > 0: queues[d2] = max(0, queues[d2] - 2.5)
                    ...
                    # During Yellow/Red, no cars depart. Delay skyrockets.
                    timer += 1
                    if timer >= (YELLOW_TIME + ALL_RED_TIME):
                        state = "GREEN"
                        timer = 0
                ...
                # Micro-step Delay Accounting
                # Every car currently stopped adds 1 second to total_delay
                for d in range(4):
                    d1, d2 = (0, 1) if current_phase == 0 else (2, 3)
                    if (d != d1 and d != d2) or state != "GREEN":
                        if queues[d] > 0:
                            total_delay += queues[d]
                            max_wait[d] += 1
                            
                # If all cars cleared, stop simulation to save time
                if spawned >= TOTAL_CARS and sum(queues) < 1.0:
                    break
                    
            # Heavily penalize massive total delays
            return 10000.0 / (total_delay + 1.0)
        except: return 0.0

    def optimize(self):
        print(f"PSO Running: Micro-step training {self.num_particles} particles, {self.iterations} generations...")
...
def draw_ui_panel(screen, font, title_font, sim_hour, intensity_factor, controller, genome, logger):
    ...
    add_text("PSO OPTIMIZED FLC", CYAN, True)
...
def main():
    ...
    pso = ParticleSwarmOptimizer(num_particles=20, iterations=15)
    ...
    pygame.display.set_caption("AI Traffic Intersection - PSO FLC Benchmark")
    ...
    while running:
        ...
        # BENCHMARK SPAWN LOGIC - ASYMMETRIC GRIDLOCK (90% Highway, 10% Side-Street)
